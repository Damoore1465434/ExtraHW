//https://gist.github.com/cfgchouhang/5553718
			...
			for (m = -w; m <= w; m++) {
				for (n = -w; n <= w; n++) {
					if (i + m >= 0 && j + n >= 0 && i + m < SAMPLE_ROW&&j + n < SAMPLE_COL) {
						...
						//SSD[i][j] += dist(s[i+m][j+n],t[m+w][n+w])*((double)vMask[m+w][n+w])*gMask[m+w][n+w];
						//printf("ssd ch %lf\n",SSD[i][j]);
					}
				}
			}
...
int main(int argc, char *argv[])
{
	...
	*(unsigned int *)(t + 0x16) = newCol;
	...
	//randRow = rand()%(SAMPLE_ROW-seedSize+1);
	randRow = SAMPLE_ROW / 3;
	randCol = SAMPLE_COL / 2;
	//randCol = rand()%(SAMPLE_COL-seedSize+1);
	...
	while (nfilled < npixels) {
		...
		while (pixel[p][0] >= 0) {
			...
			m = 0;//pick one matches
			//printf("match %d %d\n",matches[m][0],matches[m][1]);
			//printf("ssd %.10lf\n",SSD[matches[m][0]][matches[m][1]]);
